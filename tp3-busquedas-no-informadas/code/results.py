...
# 1. El entorno del agente está compuesto por  una grilla de 100x100 en donde los obstáculos se generan al azar. Se trata de un entorno completamente observable, determinista 
#y estático.
...
# Ejecutar un total de 30 veces cada algoritmo en un escenario aleatorio con una tasa de obstáculos del 8 por ciento, calcular la media y la desviación estándar de la
# cantidad de estados explorados para llegar al destino (si es que fue posible). Evaluar cada uno de los algoritmos sobre el mismo conjunto de datos generado.  Presentar
# los resultados en un gráfico de cajas y bigotes o boxplots.

#Dentro tp3-busquedas-no-informadas crear un archivo de nombre no-informada-results.csv en formato csv (comma separated values) con los resultados de las 30 ejecuciones 
#para cada uno de los algoritmos evaluados. El formato deberá ser el siguiente: agent_name, run_n, estate_n, solution_found
...
for agent_name in agent_names:
    ...
    path_cost_filtered_results = [result["path_cost"] for result in results if result["agent_name"] == agent_name and result["path_cost"] is not None]
